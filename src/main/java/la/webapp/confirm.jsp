...
<body>
	...
	<c:if test="${not empty cart.items}">
		<table border="1">
			...
			<tr>
				<td align="right" colspan="6"><c:if
						test="${cart.total lt 10000}">
合計金額:<fmt:formatNumber value="${cart.total}" />円
</c:if> <c:if test="${cart.total ge 10000}">
合計金額:<fmt:formatNumber value="${cart.total * 0.9}" />円
</c:if></td>
			</tr>
		</table>
		...
		<form action="/shopping/OrderServlet?action=order" method="post">
			...
			<h3>支払方法</h3>
			${message} <br> <input type="submit" value="この内容で注文">
		</form>
	</c:if>
</body>
</html>